...
from backend.schemas.responses import JobStatusResponse, NoteResponse, SemanticCardResponse, ProcessBookResponse, SemanticNetworkItem
from backend.services.llm_service import AsyncLLMService, ReferenceSearcher
...
async def run_processing_task(job_id: str, raw_text: str, full_book_text: str = "", original_note_id: str = ""):
    ...
    try:
        ...
        final_markdown = await orchestrator.process_all_notes(
            raw_text=raw_text,
            enable_l3=False,  # L3 降级为可选，用户通过前端"深度看这本"手动触发
            enable_l4=True,
...
async def _try_read_from_filesystem(note_id: str, title: str) -> str | None:
    ...
    # 兜底：也检查 raw_notes 目录（用户跳过清洗时直接看原始笔记）
    raw_dir = os.path.join(base_dir, "data", "raw_notes")
    if os.path.exists(raw_dir):
        for root, dirs, files in os.walk(raw_dir):
            for f in files:
                if f.endswith('.md'):
                    if title_no_brackets in f or title in f or (core_name and core_name in f and len(core_name) >= 2):
                        possible_paths.append(os.path.join(root, f))
...
async def get_semantic_card(note_id: str, request: SemanticCardRequest):
    ...
    try:
        # 调用大模型生成 JSON
        result_json = await llm_service.generate_json(prompt, system_msg="你是阅读同伴——记性好、联想力强，但从不假装全知。你分享观点的语气像朋友聊天，口语化、谦虚、偶尔有点可爱。")

        # 将结果缓存在 store 中以备重复读取 (可选)
        store.semantic_cards[note_id] = result_json

        # 确保返回数据格式正确
        # semantic_network 支持新格式（dict list）和旧格式（string list）
        semantic_network = result_json.get("semantic_network", [])
        network_items = []
        if semantic_network:
            for item in semantic_network:
                if isinstance(item, dict):
                    network_items.append(SemanticNetworkItem(
                        term=item.get("term", str(item)),
                        relation=item.get("relation", item.get("note", "关联")),
                        explanation=item.get("explanation", item.get("note")),
                        provocation=item.get("provocation")
                    ))
                else:
                    network_items.append(SemanticNetworkItem(term=str(item)))

        # reference_urls: 用真实搜索替换 AI 编造的 URL
        ...
        for ref in reference_urls:
            ...
            if ref_title:
                search_results = await ReferenceSearcher.search_book_references("", ref_title)
                hydrated_refs.extend(search_results)
        if not hydrated_refs:
            hydrated_refs = [{"title": f"百度一下：{term}", "url": f"https://www.baidu.com/s?wd={term}"}]
        ...
        return SemanticCardResponse(
            ...
            semantic_network=network_items,
            reference_urls=hydrated_refs[:2]
        )
    except Exception as e:
...
async def fetch_book_cover(title: str, author: str = ""):
    """
    多渠道爬取图书封面，返回第一个可用的封面 URL。
    优先顺序：Open Library → Google Books → 失败返回 null
    """
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for doc in data.get("docs", []):
                    ...
                    if cover_id:
                        return {"url": f"https://covers.openlibrary.org/b/id/{cover_id}-M.jpg", "source": "openlibrary"}
        except Exception:
            pass
        ...
        try:
            ...
            resp = await client.get(
                ...
                params={"q": query, "maxResults": 3, "langRestrict": "zh"}
            )
            if resp.status_code == 200:
                data = resp.json()
                for item in data.get("items", []):
                    ...
                    if thumb:
                        cover = thumb.replace("&edge=curl", "").replace("http://", "https://")
                        return {"url": cover, "source": "google"}
        except Exception:
            pass
        ...
        try:
            ...
            if resp.status_code == 200:
                import re
                match = re.search(r'<img[^>]+src="(https://[^"]+book/pic/[^"]+)"', resp.text)
                if match:
                    return {"url": match.group(1), "source": "douban"}
        except Exception:
            pass
