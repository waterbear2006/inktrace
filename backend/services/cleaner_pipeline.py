...
class L2FuzzyContextFixer(TextCleanerStep):
    ...
    def process(self, text: str, full_text: Optional[str] = None) -> str:
        ...
        if not full_text:
            ...
            for line in text.split('\n'):
                ...
                self.structured_blocks.append({
                    "markdown": stripped if stripped.startswith('- ') or stripped.startswith('* ') else f"- {stripped}",
                    "core_concept": clean_line[:20] + "..." if len(clean_line) > 20 else clean_line,
                    "raw_text": stripped,
        ...
        for i, curr in enumerate(lines):
            ...
            if idx != -1:
                intervals.append([idx, idx + match_len, c_curr])
        ...
        for start, end, original_text in merged:
