...
from backend.services.parsers import KindleParser, WeReadParser, LLMParser
...
class NoteRouter:
    ...
    def __init__(self):
        ...
        self.parsers = [KindleParser(), WeReadParser(), LLMParser()]
...
class NoteOrchestrator:
    ...
    def __init__(self):
        ...
        self.l4_index = get_index()
        self.exporter = MarkdownExporter()

    async def process_note_task(self, job_id: str, raw_text: str, full_book_text: str = "", enable_ai_structuring: bool = True, enable_glossary: bool = True):
        """
        后台任务入口：执行完整清洗流程并更新 store 中的 job 进度
        """
        ...
        try:
            ...
            async def update_progress(progress: int, status_msg: str = ""):
                ...
                if status_msg:
                    logger.info(f"📊 [{job_id}] {progress}% - {status_msg}")

            await update_progress(5, "开始解析原始笔记...")

            final_markdown = await self.process_all_notes(
                raw_text=raw_text,
                enable_l3=enable_ai_structuring,
                enable_l4=True,
            ...
            logger.info(f"✅ [process_note_task] 任务完成: {job_id}")

        except Exception as e:
            logger.error(f"❌ [process_note_task] 任务失败: {job_id}: {e}")
            import traceback
            traceback.print_exc()
            ...
            if enable_l3 and structured_blocks:
                ...
                for block, ext in zip(structured_blocks, extensions):
                    ...
                    if ext:
                        ...
                        md += f"original: {block.get('original_text', block.get('raw_text', '无原文对照'))[:200]}...\n"
                        md += f"tags: {', '.join(ext.tags)}\n"
                        network_items = []
                        for item in ext.semantic_network:
                            term = item.get("term", "未知概念")
                            relation = item.get("relation", item.get("note", "关联"))
                            explanation = item.get("explanation", item.get("note", "暂无详细解释"))
                            provocation = item.get("provocation", "")
                            network_items.append(f'{term}|||{relation}|||{explanation}|||{provocation}')
