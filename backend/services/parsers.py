...
# --- 3. 微信读书专用翻译官（已结构化，零 token 透传） ---
class WeReadParser(BaseNoteParser):
    def identify(self, raw_text: str) -> bool:
        # WeRead 格式特征：包含 "## 想法与思考" 章节，
        # 或以 "# 书名 - 作者" 开头且有 "## " 章节标题
        return "## 想法与思考" in raw_text or (
            raw_text.strip().startswith("# ") and "## " in raw_text
        )
