...
class MockStore:
    ...
    def _load_real_books(self):
        ...
        if os.path.exists(notes_dir):
            ...
            for filename in os.listdir(notes_dir):
                if filename.lower().endswith((".txt", ".md", ".rtf")):
                    ...
                    # 跳过 WeRead 导入的文件，由第 4 步统一处理
                    if raw_title.endswith("_WeRead"):
                        continue
        ...
        if os.path.exists(notes_dir):
            weread_count = 0
            for filename in os.listdir(notes_dir):
                if filename.endswith("_WeRead.md"):
                    raw_title = os.path.splitext(filename)[0]
                    # 去掉 _WeRead 后缀作为显示标题
                    display_title = raw_title.replace("_WeRead", "")
                    note_id = "note_" + str(uuid.uuid5(uuid.NAMESPACE_URL, raw_title))[:8]

                    is_done = any(display_title in cb or raw_title in cb for cb in completed_books)
                    ...
                    self.books[note_id] = {
                        ...
                        "source": "WeRead",
                        "progress": 100 if is_done else 0,
                        "status": "已完成" if is_done else "待清洗",
                        "statusColor": "bg-green-500" if is_done else "bg-red-500",
                        "isError": not is_done,
                        "img": ""
                    }
                    weread_count += 1
            if weread_count:
                print(f"✅ 已加载 {weread_count} 本微信读书导入")
